...
const BidderDashboard = () => {
  ...
  const [auction, setAuction] = useState(null);
  const [bids, setBids] = useState([]);
  const [loading, setLoading] = useState(false); // Start false, set true only when needed
  const [error, setError] = useState(null);
  const socketRef = useRef(null);

  useEffect(() => {
    if (!id) {
      setLoading(false); // No loading for non-auction routes
      return;
    }
    ...
    socketRef.current = newSocket;
    setLoading(true); // Set loading only when fetching starts
    ...
    const fetchAuctionDetails = async () => {
      try {
        ...
        console.error("Error fetching auction details:", err.message, err.response?.status, err.response?.data);
        setError(`Failed to load auction details: ${err.message} (Status: ${err.response?.status})`);
      }
    };
    ...
    const fetchAuctionBids = async () => {
      try {
        ...
        console.error("Error fetching bids for auction:", err.message, err.response?.status, err.response?.data);
        setError((prev) => prev ? `${prev}\nFailed to load bid history: ${err.message} (Status: ${err.response?.status})` : `Failed to load bid history: ${err.message} (Status: ${err.response?.status})`);
      }
    };

    fetchAuctionDetails();
    fetchAuctionBids().then(() => setLoading(false)); // Ensure loading stops after both calls
                ...
                <ul className="bids-list">
                  {bids.map((bid, index) => (
                    <li key={index} className="bid-item">
                      <span className="bidder-name">{bid.bidder?.name || "Unknown"}</span>
                      <span className="bid-amount">${bid.amount}</span>
                      <span className="bid-time">{new Date(bid.timeStamp || bid.bidTime).toLocaleString()}</span>
                    </li>
                  ))}
                </ul>
              )}
